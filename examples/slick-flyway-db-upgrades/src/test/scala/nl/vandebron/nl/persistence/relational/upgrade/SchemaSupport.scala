...
trait SchemaSupport {
  ...
  case class PrimaryKey(p: MPrimaryKey) extends Entity {
    ...
    override def toString: String =
      s"ALTER TABLE ${p.table.schema.map(s => s"$s.").getOrElse("")}${p.table.name} ADD CONSTRAINT '${p.pkName
  ...
  case class Index(i: MIndexInfo) extends Entity {
    ...
    override def toString: String = {
      ...
      s"ALTER TABLE ${i.table.schema.map(s => s"$s.").getOrElse("")}${i.table.name} ADD CONSTRAINT '${i.indexName
  ...
  case class ForeignKey(fk: MForeignKey) extends Entity {
    ...
    override def toString: String = {
      ...
      s"""
        |ALTER TABLE "${fk.fkTable.name}" ADD CONSTRAINT
        |"${fk.fkName.getOrElse("{name}")}" FOREIGN KEY ("${fk.fkColumn}") REFERENCES
        |${fk.pkTable.schema.map(x => s"$x.").getOrElse("")}"${fk.pkTable.name}"("${fk.pkColumn}")$deleteAction$updateAction
        |""".stripMargin
    }
      ...
      bothEntities.foreach { c: String =>
        ...
        if (thisEntity != otherEntity) {
          if (thisEntity.toString == otherEntity.toString)
            result.append(s"\n😱 .toString is identical while .equals is not for $thisEntity")
  ...
  case class Table(
    ...
    def diff(other: Table): String = {
      val result = new mutable.StringBuilder(s"👉 Table '$name'")
