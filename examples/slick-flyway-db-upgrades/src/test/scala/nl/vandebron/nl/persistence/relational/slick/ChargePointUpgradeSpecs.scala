...
import nl.vandebron.persistence.relational.slick.{ ChargePointTable, Connection }
...
class ChargePointUpgradeSpecs
    extends SchemaRoundtripSpecs(
      db = Connection.db,
      toCreate = Seq(TableQuery[ChargePointTable]).map(_.asInstanceOf[TableQuery[RelationalProfile#Table[_]]]),
      createDatabase = () => 0,
